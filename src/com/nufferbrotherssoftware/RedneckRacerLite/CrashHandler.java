					...
					final AsyncTask task = new AsyncTask<Void,Void,Void>() {
                        ...
                        protected Void doInBackground(Void... v) {
                            try {
                                ...
                                log = StartActivity.readAllOf(process.getInputStream());
                            } catch (IOException e) {
                                e.printStackTrace();
                        ...
                        protected void onPostExecute(Void v) {
                            ...
                            // MADD
                            boolean ok = StartActivity.tryEmailAuthor(CrashHandler.this, true,
                                getString(R.string.crash_preamble)+"\n\n\n\nLog:\n"+
                                                                      log.substring(log.length() > 20000 ? log.length() - 20000 : 0));
                            progress.dismiss();
                            if (ok) {
                    ...
                    b.postDelayed(new Runnable() {
                        public void run() {
                            ...
                            new AlertDialog.Builder(CrashHandler.this)
                                .setMessage(MessageFormat.format(getString(R.string.get_log_failed), 
